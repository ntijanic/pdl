...
def print_prepare(arg):
    return json.dumps(arg) if not arg.isalnum() else arg
...
def assert_process_fails(exit_code, str_to_search=""):
    ...
    def decorator(func):
        ...
        def wrapped():
            try:
                ...
                assert exit_code == e.proc.exit_code, 'Exit code: %s. Expected: %s' % (exit_code, e.proc.exit_code)
